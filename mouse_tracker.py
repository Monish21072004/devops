...
# Conditionally import pynput.mouse if mouse tracking is enabled.
if os.getenv("DISABLE_MOUSE_TRACKER", "false").lower() != "true":
...
class MouseBehaviorTracker:
    ...
    def start(self):
        if mouse is None:
            self.logger.info("Mouse tracking disabled by environment; not starting listener.")
            return
