...
import face_detector  # Updated face detector module.
from voice_detector import VoiceDetector  # Voice detector module.
...
# Initialize and calibrate the voice detector.
...
def graph_event(event_type):
    ...
    for event in events:
        ...
        if t is not None:
            times.append(t)
            # For face events, use the risk value; otherwise, default to 1.
...
if __name__ == '__main__':
    ...
    # Start each tracker/detector in its own thread.
    ...
    # Bind to 0.0.0.0 so it listens on all interfaces.
